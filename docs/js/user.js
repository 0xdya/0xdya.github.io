...
async function initVisits() {
  ...
  if (counterEl) counterEl.innerText = count.toLocaleString('en-US');
}
...
async function fetchCounts() {
  try {
    ...
    if (usersCountEl) usersCountEl.innerText = usersSnap.data().count.toLocaleString('en-US');

    const commentsCountEl = document.getElementById("comments_count");
    if (commentsCountEl) commentsCountEl.innerText = commentsSnap.data().count.toLocaleString('en-US');
  } catch (error) {
